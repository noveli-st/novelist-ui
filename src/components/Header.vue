            ...
            <div class="ml-auto d-flex align-items-center">
                ...
                <div v-if="isAuthenticated" class="position-relative d-inline-block mx-2">
                    ...
                    <b-collapse ref="userMainMenu" id="userMainMenu" class="position-absolute r-0 mt-1 rounded overflow-hidden shadow-sm bg-white z-index-1" style="min-width: 320px; max-width: 320px;" v-click-outside="closeMainMenu">
                        <router-link class="d-flex btn btn-light btn-block border-0 rounded-0 m-0 p-3 text-decoration-none disable-events" active-class="active" v-bind:to="`profile/${ this.$store.state.user.me.id }`">
                            <font-awesome-icon icon="id-card" class="my-auto" /><span class="ml-auto pl-2 text-truncate">{{ this.$store.state.user.me.name }}</span>
                        </router-link>
                        <!-- <hr class="my-0"> -->
                        <router-link class="btn btn-outline-secondary btn-block border-0 rounded-0 text-left text-nowrap m-0 px-3 py-3 disable-events disabled" active-class="active" v-bind:to="'wallet'" v-on:click.native="closeMainMenu"><font-awesome-icon icon="wallet" class="mr-2" />Wallet <span class="float-right">$201.00</span></router-link>
                        <div class="d-flex flex-wrap">
                            <router-link class="btn btn-outline-secondary border-0 rounded-0 m-0 p-3 col-4 disable-events disabled" active-class="active" v-bind:to="'my-library'" v-on:click.native="closeMainMenu"><font-awesome-icon icon="book-reader" size="2x" /><div class="small">Library</div></router-link>
                            <router-link class="btn btn-outline-secondary border-0 rounded-0 m-0 p-3 col-4 disable-events disabled" active-class="active" v-bind:to="'my-reviews'" v-on:click.native="closeMainMenu"><font-awesome-icon icon="glasses" size="2x" /><div class="small">Reviews</div></router-link>
                            ...
                            <router-link class="btn btn-outline-secondary border-0 rounded-0 m-0 p-3 col-4 disable-events disabled" active-class="active" v-bind:to="'my-comments'" v-on:click.native="closeMainMenu"><font-awesome-icon icon="comments" size="2x" /><div class="small">Comments</div></router-link>
                            <router-link class="btn btn-outline-secondary border-0 rounded-0 m-0 p-3 col-4 disable-events disabled" active-class="active" v-bind:to="'my-statistics'" v-on:click.native="closeMainMenu"><font-awesome-icon icon="chart-area" size="2x" /><div class="small">Stats</div></router-link>
                        </div>
                        <hr class="my-0">
                        <div class="d-flex flex-wrap">
                            <router-link class="btn btn-outline-secondary border-0 rounded-0 m-0 p-2 col-3 disable-events disabled" active-class="active" v-bind:to="'inbox'" v-on:click.native="closeMainMenu"><font-awesome-icon icon="inbox" size="lg" /><div class="small text-truncate">Inbox</div></router-link>
                            ...
                            <router-link class="btn btn-outline-secondary border-0 rounded-0 m-0 p-2 col-3 disable-events disabled" active-class="active" v-bind:to="'get-help'" v-on:click.native="closeMainMenu"><font-awesome-icon icon="question-circle" size="lg" /><div class="small text-truncate">Get help</div></router-link>
                        </div>
                        <!-- <hr class="my-0"> -->
